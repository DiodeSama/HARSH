...
# test_files = sorted(glob.glob(os.path.join(args.poisoned_dir, "poisoned_test_batch_*.pt")))
# test_files = sorted(glob.glob(os.path.join(args.poisoned_dir, "resnet_square_poisoned_test_batch_*.pt")))
# test_files = sorted(glob.glob(os.path.join(args.poisoned_dir, "resnet_sig_poisoned_test_batch_*.pt")))
# test_files = sorted(glob.glob(os.path.join(args.poisoned_dir, "resnet_blend_poisoned_test_batch_*.pt")))
test_files = sorted(glob.glob(os.path.join(args.poisoned_dir, "resnet_ftrojan_poisoned_test_batch_*.pt")))
# test_files = sorted(glob.glob(os.path.join(args.poisoned_dir, "resnet_HCB_poisoned_test_batch_*.pt")))
# test_files = sorted(glob.glob(os.path.join(args.poisoned_dir, "resnet_HCB_poisoned_test_batch_*.pt")))
imgs, labels = [], []
for f in test_files:
    batch = torch.load(f, map_location=device)
...
poisoned_acc, poisoned_loss = evaluate(model, poisoned_loader, smoothing_type="no_smooth")
print(f"Poisoned Accuracy: {100.0 * poisoned_acc:.2f}% | Loss: {poisoned_loss:.4f}")

# ------------------ Evaluate on Clean ------------------
print("\nEvaluating on clean CelebA test set...")
clean_acc, clean_loss = evaluate(model, clean_loader, smoothing_type="no_smooth")
...
#   --ckpt_path /mnt/sdb/models/train_attack_HCB_resnet_celeba_0.1_HCB_no_smooth_epoch2.pt \
#   --model resnet \
#   --data celeba \
#   --num_classes 8 \
#   --device cuda:0 \
...
# python eval_attack.py \
#   --ckpt_path /home/suser/project/MOTH/ckpt/celeba_resnet18_train_attack_HCBsmile_resnet_celeba_0.1_HCBsmile_no_smooth_epoch3.pt_moth.pt\
#   --model resnet \
#   --data celeba \
#   --num_classes 8 \
#   --device cuda:0 \
