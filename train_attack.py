...
def test(model, test_loader):
    ...
    for batch_idx, (data, label) in enumerate(test_loader):
        save_path = f'/mnt/sdb/dataset_checkpoint/{args.model}_{args.attack_mode}_poisoned_test_batch_{batch_idx}.pt'
        if test_index == 0:
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, label) in enumerate(train_loader):
            save_path = f'/mnt/sdb/dataset_checkpoint/{args.model}_{args.attack_mode}_poisoned_batch_{batch_idx}.pt'
            if os.path.exists(save_path):
                poisoned = torch.load(save_path)
                data = poisoned['data']
                label = poisoned['label']
